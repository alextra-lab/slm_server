...
- **`router.py`** — FastAPI app with shared `httpx.AsyncClient` (connection pooling). `/v1/responses` auto-falls back to chat/completions on 404. Errors returned in OpenAI format with `slm_server_debug` metadata.
