...
def test_a_backend_with_a_missing_draft_does_not_stop_the_next_one(
    monkeypatch, tmp_path: Path
) -> None:
    """One bad spec_model_path once ended the launcher before later backends started."""
    ...
    good = ModelDefinition(id="test/good", port=8602, **common)
    cfg = ModelConfig(models={"bad": bad, "good": good})
    ...
    # Stop before supervision: this test is about the start loop only.
    monkeypatch.setattr(wd.BackendSupervisor, "run", lambda self, should_continue=None: None)

    with pytest.raises(SystemExit):
        start_backends.main()

    assert len(launched) == 1
    assert "8602" in launched[0]
